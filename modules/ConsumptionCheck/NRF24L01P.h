 ...
 */
class NRF24L01P : protected SPI::Driver {
public:
...
//	static const uint8_t BROADCAST = 0x00;
	...
	NRF24L01P(uint8_t server, 
			Board::DigitalPin power,
	...
	NRF24L01P(uint8_t server, 
			...
			Board::ExternalInterruptPin irq = Board::EXT0)
#endif
	:SPI::Driver(csn, SPI::ACTIVE_LOW, SPI::DIV4_CLOCK, 0, SPI::MSB_ORDER, &_irq),
	_server(server),
	_power(power),
	_ce(ce, 0),
	_irq(irq, ExternalInterrupt::ON_FALLING_MODE, this),
	_status(0),
	 ...
	 */
	int recv(uint8_t expected_port, void* buf, size_t count, uint32_t ms = RECV_TIMEOUT_MS);

	/**
	 * Set output power level (-30..10 dBm)
	 * @param[in] dBm.
	 */
	void output_power_level(int8_t dBm);
...
protected:
	...
	OutputPin _ce; //!< Chip enable activity RX/TX select pin.
	IRQPin _irq; //!< Chip interrupt pin and handler.
